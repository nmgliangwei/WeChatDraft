...
- **排版美化**：可选开关。开启后用 DOMDocument 给正文 HTML 注入 inline style（微信编辑器只认 inline style），让标题、列表、表格、引用、代码块在公众号里的排版风格接近 markdown 编辑器，主题色可配。
...
微信公众号编辑器**只认 inline style**，不识别 `<style>` 标签或外链 CSS。本插件可选地用 DOMDocument 在提交草稿前给正文 HTML 注入 inline style，让排版风格接近 markdown 编辑器：

| 配置项 | 默认 | 说明 |
|---|---|---|
| **排版美化** | 不勾选 | 勾选后才走美化流程。**默认关闭**，避免影响已经依赖博客主题样式的老用户。 |
| **美化主题色** | `hsl(216, 100%, 68%)` | 用于 h1/h2 描边、blockquote 左侧竖线、inline code 文字、strong 加粗。任何合法 CSS 颜色都行，如 `#1e88e5`、`rgb(255, 99, 71)`。 |

处理范围（按标签）：

- `h1`–`h6`：标题分级，h1/h2/h6 用主题色描边，h3–h5 调整字号字重
- `p`：基础字号 16px、行高 26px、配色 #333
- `ul` / `ol` / `li`：列表缩进 + 圆点 / 数字
- `blockquote`：浅灰背景 + 主题色左竖线
- `table` / `thead` / `th` / `td`：表格边框、内边距、表头底色（微信原生表格几乎没样式）
- `<code>`（行内）：圆角浅灰背景 + 主题色文字 + 等宽字体
- `<pre><code>`（代码块）：包装成微信编辑器同款 `code-snippet__fix` 两栏结构（左行号 + 右代码）
- `strong` / `hr`：加粗用主题色、分隔线改虚线
- 整体：外层包一层 `<section id="nice">` 设全文字体（PingFang SC / Microsoft YaHei）与行高

**注入策略 — 不破坏博客已有样式**：插件追加的 style 放在原 `style` 属性之前。如果原 HTML 上已经有同名样式声明（比如博客主题已经给 `<p>` 设了字体），CSS「后写优先」规则会让你的原值胜出，插件只在原本没设置的属性上补足。

代码块没有引入 GeSHi 等高亮库（库太老、体积大），等宽字体由微信公众号客户端原生渲染。
